...
export class ImageProcessor {
            ...
            if (customEdgesMapImgPath) {
                ...
                fs.createReadStream(customEdgesMapImgPath)
                    ...
                    .on('error', (error) => {
                        ...
                        logger.logError(
                            `Could not load the customEdgesMap picture, make sure image is valid PNG file.\n${
                                error.message
                            }`,
                        );
                    });
    ...
    private edgesMapImage: PNG;

    private constructor(imgData: PNG) {
        this.edgesMapImage = imgData;
        imgData
            .pack()
            .pipe(fs.createWriteStream('currentlyMaking.png'))
            .on('close', () => {
                logger.log(
    ...
    public getIncrementalEdges(
        intensityValueMin: number,
        intensityValueMax: number,
    ): { x: number; y: number }[] {
        const resultCoordsArray: { x: number; y: number }[] = [];

        for (let y = 0; y < this.edgesMapImage.height; y++) {
            for (let x = 0; x < this.edgesMapImage.width; x++) {
                // tslint:disable-next-line: no-bitwise
                const idx = (this.edgesMapImage.width * y + x) << 2;

                const r = this.edgesMapImage.data[idx + 0];
                const g = this.edgesMapImage.data[idx + 1];
                const b = this.edgesMapImage.data[idx + 2];
                const a = this.edgesMapImage.data[idx + 3];
